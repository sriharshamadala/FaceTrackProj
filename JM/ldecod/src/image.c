...
//Insert the node which store motion vectors of one frame into the Linked List
...
void decode_slice(Slice *currSlice, int current_header,frame_mv *fmv)
{
  ...
  if ((current_header == SOP || current_header == SOS) && currSlice->ei_flag == 0)
    decode_one_slice(currSlice,fmv);
...
{
  struct fmvnode *frame_node=(struct fmvnode*)malloc(sizeof(struct fmvnode));  //a node storing motion vectors of one frame in the Linked List
  ...
  frame_node->fmv=(frame_mv *)malloc(p_Vid->PicSizeInMbs*sizeof(frame_mv));//fmv is an array whose element type is frame_mv and size is the number of macroblocks in one frame
  {
    for(iSliceNo=0; iSliceNo<p_Vid->iSliceNumOfCurrPic; iSliceNo++)
    {
      ...
      decode_slice(currSlice, current_header,frame_node->fmv);
  ...
  frame_node->poc=p_Vid->dec_picture->poc; 
  frame_node->next=NULL;
  mv_insert(frame_node,mvlist);  //Insert the node which store motion vectors of one frame into the Linked List
 ...
 ************************************************************************
 */
void decode_one_slice(Slice *currSlice,frame_mv *fmv)
{
  int a,b,c;
  VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
	...
	for(a=0;a<2;a++)   
		for(b=0;b<4;b++)
			for(c=0;c<4;c++)
			{                           //extract motion vectors into our array
					fmv[currMB->mbAddrX].mv[a][b][c][0]=((*((*((*currMB).p_Slice)).dec_picture)).mv_info[currMB->block_y+b][currMB->block_x+c]).mv[a].mv_x;
					fmv[currMB->mbAddrX].mv[a][b][c][1]=((*((*((*currMB).p_Slice)).dec_picture)).mv_info[currMB->block_y+b][currMB->block_x+c]).mv[a].mv_y;
