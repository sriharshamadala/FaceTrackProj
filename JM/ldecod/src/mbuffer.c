...
#include "MotionVectorLinkList.h"

frame_mv *mv_del(struct mv_linklist *motionlist, int mark)
{
	struct fmvnode *last=NULL;
	struct fmvnode *current=motionlist->head;
	frame_mv *get=NULL;
		while(current!=NULL)
		{
			if(current->poc==mark)
...
extern struct mv_linklist *mvlist;
struct pixel
{
	int x;
	int y;
...
struct node  
{
    struct pixel info;
    struct node *link;
}*front=NULL,*rear=NULL;    //the queue for graph BFS
struct node2  //small box
{
	struct pixel tl; //top left pixel of the small box
	struct pixel br;
	struct node2 *next;   //when plotting the big box, this pointer points to a small box connected to the current one
};

struct node3    //the node in the Linked List which include all the small boxes
{
	struct node2 data;
	struct node3 *next;
};
struct list_type3  //Linked List whose node is the small box
{
	struct node3 *head;
	struct node3 *tail;
	int length;
};
void DeleteList(struct node3 *head)
{
  struct node3 *next, *deleteMe;
  deleteMe = head;
  while (deleteMe) 
  {
    next = deleteMe->next;
...
void insert_list(int x,int y,int b,int c,struct pixel pxy,struct list_type3 *list) 
{
	struct node3 *new_node;
	new_node=(struct node3 *)malloc(sizeof(struct node3));
    new_node->data.tl.x=16*x+4*c;
	new_node->data.tl.y=16*y+4*b;
	new_node->data.br.x=16*x+4*pxy.x+3;
	new_node->data.br.y=16*y+4*pxy.y+3;
	new_node->data.next=NULL;
	new_node->next=NULL;
	 if(list->length<=0)
		{
			list->head=new_node;
...
double myDistance(struct pixel tl1, struct pixel tl2)
{
	double dist=((tl1.x-tl2.x)*(tl1.x-tl2.x)+(tl1.y-tl2.y)*(tl1.y-tl2.y));
	return dist;
	//double dist;
	//dist = sqrt(pow(double(tl1.x-tl2.x),2) + pow(double(tl1.y-tl2.y),2));

}
int isEmpty()  //judge whether a queue is empty
{
    if(front==NULL)
        return 1;
    else
...
void insert(int item1, int item2)  //insert a 4x4 submacroblock into a queue when doing graph BFS
{
    struct node *tmp;
...
struct pixel del()  //dequeue when doing graph BFS
{
    struct node *tmp;
    struct pixel item;
    if( isEmpty( ) )
...
struct pixel bfs(int i,int a,int b,int c,int visited[4][4],frame_mv *fmv)   //graph BFS to merge 4x4 submacroblock
{
	struct pixel pxy,next;
	visited[b][c]=1;
	insert(b,c);
	while(!isEmpty())
	{
		pxy=del();
		next.x=pxy.x+1;next.y=pxy.y;
		if((fmv[i].mv[a][b][c][0]==fmv[i].mv[a][next.y][next.x][0])&&(fmv[i].mv[a][b][c][1]==fmv[i].mv[a][next.y][next.x][1]))
		{
			if(visited[next.y][next.x]==0)
			{
				visited[next.y][next.x]=1;
				insert(next.y,next.x);
			}
		}
		next.x=pxy.x;next.y=pxy.y+1;
			if((fmv[i].mv[a][b][c][0]==fmv[i].mv[a][next.y][next.x][0])&&(fmv[i].mv[a][b][c][1]==fmv[i].mv[a][next.y][next.x][1]))
				if(visited[next.y][next.x]==0)
			{
				visited[next.y][next.x]=1;
...
void plot_box(int x,int y,int b,int c, struct pixel pxy, imgpel **Y)
{
	int spx=16*x+4*c;
	int spy=16*y+4*b;
	int epx=16*x+4*pxy.x;
	int epy=16*y+4*pxy.y;
	int i,j;
	for(j=spy;j<epy+4;j++)
	{
		if((j==spy)||(j==epy+3))
...
void rec_box(int x,int y,frame_mv *fmv,imgpel **Y,struct list_type3 * list,int MVthresh,int row_length)
{
	int i,a,b,c;
	struct pixel pxy;
	...
	for(b=0;b<4;b++)
			 for(c=0;c<4;c++)
				 if(((fmv[i].mv[0][b][c][0]*fmv[i].mv[0][b][c][0]+fmv[i].mv[0][b][c][1]*fmv[i].mv[0][b][c][1]>=MVthresh)||(fmv[i].mv[1][b][c][0]*fmv[i].mv[1][b][c][0]+fmv[i].mv[1][b][c][1]*fmv[i].mv[1][b][c][1]>=MVthresh))&&(visited[b][c]==0))
				{
						 pxy=bfs(i,a,b,c,visited,fmv);        //find the macroblock partition and merge them into a small box
						 //plot_box(x,y,b,c,pxy,Y);
						 insert_list(x,y,b,c,pxy,list);   //insert the small box into a Linked List
				 }
				 
}
void plot_big_box(imgpel **Y, struct list_type3 * list, double distTH)
{
	int rootcount=0;
	int lone;
	int i,j;
	struct node2 *temp=NULL;
	struct node2 *temp2, *temp3;
	struct node2 **roots;
	int temp4=0;
	int listNodePresentIn;
	int maxListNum;
	...
	int zero=0;
	struct node3 *copy;
	roots=(struct node2 **)malloc(sizeof(struct node2 *)*(list->length));
	roots[0] = &(list->head->data);
	rootcount++;
	
	for (copy=list->head->next; copy!=NULL;copy=copy->next)
	{
		lone = 1;
		/*for (temp6=0; temp6<rootcount; temp6++)
		{
			temp7 = roots[temp6];
			while(temp7->next != NULL){
				printf("(%d, %d) ", temp7->tl.x, temp7->tl.y); 
				temp7 = temp7->next;
			}
			printf("(%d, %d)\n", temp7->tl.x, temp7->tl.y);
		...
		for (j=0; j<rootcount; j++)
		{
			...
			while( temp != NULL)
			{
				if (myDistance(copy->data.tl, temp->tl)<= distTH)
				{
					if (lone==0)
					{
						//This node links two seperate lists.
						//remove the root of the second tree and link it to the prev existing tree.
						temp3 = roots[listNodePresentIn];
						while(temp3->next != NULL)
						{
							temp3 = temp3->next; 
							//printf("%d,%d ",temp3->tl.x,temp3->tl.y);
						}
							//travel to the end of this list
						temp3->next = roots[j];
						//adjust the roots array since we removed a list from the roots.
						temp4 = 0;
						while (temp4 < (rootcount-j-1))
						{
							roots[j+temp4] = roots[j+temp4+1];
							temp4++;
						}
						rootcount--;
						break;
					...
					else
					{
						lone = 0;
					listNodePresentIn = j;
					//insert this node in this list
					temp2 = temp->next;
					temp->next =&( copy->data);
					copy->data.next = temp2;
					//insert successful. quit searching this list.
					break;
					}
				}
				//else
					//printf("Found a box outside current list\n");
				temp = temp->next;
			}
		}
		if (lone==1)
		{
			...
			roots[rootcount] = &(copy->data);
			rootcount++;
		}
	}

	//Now draw the box around the largest list
	maxListNum=0;
	maxListLen=0;
	//int xmax, xmin, ymax, ymin;
	
	xmax = (int*)malloc(rootcount*sizeof(int));
	memset(xmax, 0, rootcount*sizeof(int));

	xmin = (int*)malloc(rootcount*sizeof(int));
	memset(xmin, 100, rootcount*sizeof(int));

	ymax = (int*)malloc(rootcount*sizeof(int));
	memset(ymax, 0, rootcount*sizeof(int));

	ymin = (int*)malloc(rootcount*sizeof(int));
	memset(ymin, 100, rootcount*sizeof(int));
	
	///Traverse each list calculating the four parameters
	for(temp4=0; temp4<rootcount; temp4++)
	{
		listLen = 0;
		temp3 = roots[temp4];
		while(temp3 != NULL)
		{
			listLen++;
			if(temp3->tl.x < *(xmin+temp4))
				*(xmin+temp4) = temp3->tl.x;
			if(temp3->tl.y < *(ymin+temp4))
				*(ymin+temp4) = temp3->tl.y;
			if(temp3->br.x > *(xmax+temp4))
				*(xmax+temp4) = temp3->br.x;
			if(temp3->br.y > *(ymax+temp4))
				*(ymax+temp4) = temp3->br.y;
			temp3 = temp3->next;
		}
	...
	printf("Number of seperate trees: %d\n", rootcount);
	/// call plot box command with xmax[maxListNum].... here
	/*for (k=0; k<rootcount; k++)
	{*/
	k = maxListNum;
	spx=*(xmin+k);	//maxListNum
	spy=*(ymin+k);
	epx=*(xmax+k);
	epy=*(ymax+k);
	...
	{
		free(roots[j]);roots[j]=NULL;
	}*/
	free(roots);
	roots=NULL;
}	
void judge_forward_or_backward(frame_mv *filemv,int mbs)
{
	int i,b,c,a;
	for(i=0;i<mbs;i++)
	{
		...
		for(b=0;b<4;b++)
			 for(c=0;c<4;c++)
				 if((filemv[i].mv[0][b][c][0]!=0)||(filemv[i].mv[0][b][c][1]!=0))
				{ 
						a=0;
	                  break;
				 }
...
int compute_thresh(frame_mv *filemv,int mbs)
{
	int i,b,c,t,a,norm;
	int max=0;
	int tmax=0;
	...
	for(i=0;i<mbs;i++)
	{
		...
		for(b=0;b<4;b++)
			for(c=0;c<4;c++)
              if(norm=(filemv[i].mv[a][b][c][0]*filemv[i].mv[a][b][c][0]+filemv[i].mv[a][b][c][1]*filemv[i].mv[a][b][c][1])>=10)
				{
				  t=norm/5;
				  mm[(t>=10)?8:t-2]++;
				}
...
int search_nonzero(int x,int y,frame_mv *fmv,int MVthresh,int row_length)
{
	int i,a,b,c;
	int f=0;
	i=row_length*y+x;
     ...
     for(b=0;b<4;b++)
	{
			 for(c=0;c<4;c++)
				 ...
				 if((fmv[i].mv[0][b][c][0]*fmv[i].mv[0][b][c][0]+fmv[i].mv[0][b][c][1]*fmv[i].mv[0][b][c][1]>=MVthresh)||(fmv[i].mv[1][b][c][0]*fmv[i].mv[1][b][c][0]+fmv[i].mv[1][b][c][1]*fmv[i].mv[1][b][c][1]>=MVthresh))
				   {
					   f=1;
					   break;
				 }
...
static int output_one_frame_from_dpb(DecodedPictureBuffer *p_Dpb)
{
  ...
  frame_mv *filemv;
  int x,y;
  int max=0;  //norm threshold
  double distTH;   //distance threshold
  struct list_type3 *list=(struct list_type3 *)malloc(sizeof(struct list_type3)); //Linked List which include the small boxes 
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  int poc, pos;
  ...
  filemv=mv_del(mvlist,poc); //get the motion vectors of one frame according to its poc frome the motion vector Linked List 
  distTH = 512; //distance threshold to group small boxed into a big box
...
// JVT-P072 ends
judge_forward_or_backward(filemv,p_Vid->PicSizeInMbs); //decide forward or backward motion vector to use for each macroblock
max=compute_thresh(filemv,p_Vid->PicSizeInMbs);	 //compute the norm threshold to draw boxes
for(y=0;y<p_Vid->height/16;y++)
{
	 for(x=0;x<p_Vid->width/16;x++)
		 if(search_nonzero(x,y,filemv,max,p_Vid->width/16)!=0)    //search the macroblock which contains the submacroblock which has motion vector greater than the threshold
		 {
			 rec_box(x,y,filemv, p_Dpb->fs[pos]->frame->imgY,list,max,p_Vid->width/16);  //collect small boxes
			 
		 }
}
if(list->length>0)
	plot_big_box(p_Dpb->fs[pos]->frame->imgY,list, distTH);  //plot the big box
free(filemv);
filemv=NULL;
DeleteList(list->head);  //delete the Linked List which include all the small boxes to free memory
//free(list);
//list=NULL;
