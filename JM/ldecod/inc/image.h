...
#include "mbuffer.h"
typedef struct framemv  //a structure including motion vectors of one macroblock
{
	short mv[2][BLOCK_MULTIPLE][BLOCK_MULTIPLE][2];
	int fb;  //use forward or backward MV for detection
}frame_mv;
...
extern void decode_one_slice  (Slice *currSlice,frame_mv *fmv);
...
extern void decode_slice(Slice *currSlice, int current_header,frame_mv *fmv);
